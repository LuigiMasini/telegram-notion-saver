...
const selectPage = (pageId, ctx) => {
	...
	const {pages, ...userData} = data
	
	const page = pages.filter( ({id}) => id === pageId)[0]
	
	ctx.editMessageText("Selected page: "+page.title)
...
const selectTemplate = (templateId, ctx) => {
	...
	const templateData = data.templates.filter(({id})=>id===templateId)[0]
	...
	.then(({result})=>{
		...
		cache.set(ctx.chat.id.toString(), {templateData, workspaces:result})
...
// 	db.promise('SELECT t.id, t.userTemplateNumber, p.pageType, p.icon, tc.id as chatId FROM Templates as t LEFT OUTER JOIN NotionPages as p ON p.id=t.pageId LEFT OUTER JOIN TelegramChats as tc ON tc.id = p.chatId WHERE tc.telegramChatId=? ORDER BY t.userTemplateNumber',[ctx.chat.id])
	...
	db.promise('SELECT t.id, t.userTemplateNumber, tc.id as chatId FROM Templates as t LEFT OUTER JOIN TelegramChats as tc ON tc.id = t.chatId WHERE tc.telegramChatId=? ORDER BY t.userTemplateNumber',[ctx.chat.id])
	.then(({result})=>{
